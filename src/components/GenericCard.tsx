...
export default function GenericCard({ title, description, end, imgSrc='/card_icons/what-we-do.svg', imgPos='top', className='', href }: GenericCardProps) {
    ...
    const cardClasses = `flex ${
        imgPos === "top" ? "flex-col space-y-8" : "flex-row space-x-8 items-center"
      } group border border-black p-8 shadow-lg rounded-xl transition ease-in-out delay-50 duration-300 ${
        href ? "cursor-pointer" : ""
      } ${className}`;
    ...
    const cardContent = (
      ...
      return href ? <Link href={href} className={cardClasses}>{cardContent}</Link> : <div className={cardClasses}>{cardContent}</div>; // if no href is passed in, do not make card clickable
}
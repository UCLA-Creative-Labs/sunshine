...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!roleName.includes('lead') && !roleName.includes('manager')) {
            return NextResponse.json(
                { error: 'You do not have permission to sync GitHub issues for this project.' },
                { status: 403 },
            );
        }
        ...
        supabase.from('activity_log').insert({
            user_id: user.id,
            action: `Updated GitHub issue #${task.github_issue_number} for task "${task.name}"`,
        ...
        console.error('Unexpected error in update-issue route:', err);
        return NextResponse.json(

...
function memberSort(x, y) {
    ...
    if (ROLES_SORTING.indexOf(x_roles[0]) < ROLES_SORTING.indexOf(y_roles[0]))
        ...
        ROLES_SORTING.indexOf(x_roles[0]) > ROLES_SORTING.indexOf(y_roles[0])
    )
        return 1;
    else {
        if (
            ROLES_SORTING.indexOf(x_roles[1]) !=
            ROLES_SORTING.indexOf(y_roles[1])
        ) {
            if (
                ROLES_SORTING.indexOf(x_roles[1]) <
                ROLES_SORTING.indexOf(y_roles[1])
        ...
        if (!x_name || !y_name) return 0;
        if (x_name > y_name) return 1;
        else if (x_name < y_name) return -1;
        else return 0;
    }
}
